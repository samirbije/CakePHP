...
class Khacl
{
    ...
    function check($aro, $aco, $axo = null)
    {
        if (!function_exists('kh_acl_check'))
            $this->_CI->load->helper('auth/khacl');
...
class KH_ACL_AXO
{
     ...
     * @access public
     */
    function create($axo, $resource = null)
    {
        /*
        ...
        if ($resource !==null) {
            $rs = $this->_CI->db->get_where($this->_Tables['acos'], array('name' => $resource), 1);
            $row  = $rs->row();

            $where['aco_id'] = $row->id;
        }

        $rs = $this->_CI->db->get_where($this->_Tables['axos'], $where);

        if ($rs->num_rows() === 0)
        {
            // Create new AXO
            return $this->_CI->db->insert($this->_Tables['axos'], $where);
        }
        else
           return false;
    }
     ...
     * @access public
     */
    function delete($axo_id)
    {

       /* // grab the axo_id so we can delete the access -> action links later on
        if (!($rs = $this->_CI->db->get_where($this->_Tables['axos'], array('id' => $axo), 1)))
            return false;
