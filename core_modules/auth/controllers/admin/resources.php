...
class Resources extends Admin_Controller
{
		...
		else
		{
			...
			if( is_null($id))
			{
				...
				if( ! $this->khacl->aco->create($name,$parent))
				{
					...
					redirect('admin/auth/resources/form');
				}
			...
			else
			{
				...
				if($this->access_control_model->resource->checkNodeIsChildOrEqual($new_parent,$node)){
					...
					redirect('admin/auth/resources/form/'.$id);
				}
	...
	function delete()
	{
		...
		{
			redirect('admin/auth/resources','location');
		}
...
/* Location: ./core_modules/auth/controllers/admin/acl_resources.php */