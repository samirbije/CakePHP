...
class Permissions extends Admin_Controller
{
	...
	function save()
	{
		...
		redirect('admin/auth/permissions','location');
	}
	...
	function ajax_fetch_actions($group,$resource)
	{
		...
		$this->load->library('auth/khacl');
		//$query = $this->access_control_model->fetch('axos');
		$query = $this->access_control_model->getAclActions($resource);
	...
	function ajax_fetch_actions_1($resource)
	{
		$query = $this->access_control_model->getAclActions($resource);

		foreach($query as $result)
		{
			print "<input type='checkbox' name='action_{$result->id}' value='{$result->name}'>{$result->name}<br>";
			print "<div id='allow_{$result->id}' class='action_item'>";
			print "<input type='radio' name='allow_{$result->id}' value='Y'>Allow";
			print "<input type='radio' name='allow_{$result->id}' value='N' checked='checked'>Deny";
			print "</div>";
		}
	}
}
